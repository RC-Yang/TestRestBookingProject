...
//且繼承這個類就不用重寫init()、destroy()，因已有預設的實作
{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (requestURI.contains("/queryBooking") || requestURI.contains("/deleteBookingRecord")
				|| requestURI.contains("/goToUpdateBookingRecordPage")
				|| requestURI.contains("/goToUpdateBookingRecord") || requestURI.contains("/goToReg")
				|| requestURI.contains("/reg") || requestURI.contains("/goToLogIn")) {
